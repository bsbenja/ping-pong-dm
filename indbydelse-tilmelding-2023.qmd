...
DataBilletKatAkt_T %>%
	kbl(table.attr = "data-quarto-disable-processing=true",
	    col.names = NA, align = "llr", escape = F,
      caption = paste0("<p>", IkonBillet_V, " <b>Billettyper</b></p><i style=font-size:80%>", length(unique(DataBilletKatAkt_T$Billettype)), " valgmuligheder ved tilmelding.</i>")) %>%
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	footnote(paste0(
    "<i style=font-size:80%>Først til mølle-princip. Tilmeldingsfrist ", DataEventAarAkt_T$EventAarFristDato_DW_DMAA_DW, ".<br>Ved tilmelding modtages bekræftelsesmail.<br>Mikrogebyrer pålægges alle priser.</i>"),
...
data.frame(
	...
	"Klokkeslæt" = c(
		paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(-90), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+465), "%H:%M")),
    paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(-90), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(-15), "%H:%M")),
    paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(-15), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+0), "%H:%M")),
    paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+0), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+180), "%H:%M")),
    paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+180), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+330), "%H:%M")),
    paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+330), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+450), "%H:%M")),
    paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+450), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+465), "%H:%M")),
    paste(
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+465), "%H:%M"),"-",
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+510), "%H:%M")),
    paste("Fra", format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(+510), "%H:%M"))),
	"Type" = c(
    ...
    paste(
    	...
    	format(unique(DataAkt_T$EventAarStartDatoTid_DW) + minutes(-30), "%H:%M."), "</b>"),
	...
	kbl(table.attr = "data-quarto-disable-processing=true",
      ...
      caption = paste0("<p>", IkonTimeglas_V, " <b>Tidsplan</b></p><i style=font-size:80%><b>Bemærk</b>: Tidsplan kan ændres en smule alt efter deltagerantal.</i>")) %>%
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	column_spec(1, width_min = "1em") %>%
	column_spec(2, width_min = "7em") %>%
	row_spec(0, background = DataEventAarAkt_T$EventAarFarve1_RD, color = "#FFFFFF") %>%
	row_spec(6, bold = T) %>%
	row_spec(c(1, 9), bold = T, background = DataEventAarAkt_T$EventAarFarve2_RD, extra_css = "border-top:0.7px solid #999999;border-bottom:0.7px solid #999999") %>%
...
data.frame(
  ...
  check.names = F) %>%
  kbl(table.attr = "data-quarto-disable-processing=true",
      col.names = NULL, align = "l", escape = F,
      caption = paste(IkonPerson_V, "<b>Kontaktperson</b><br><i>Evt. spørgsmål til kontaktperson</i>")) %>%
...
data.frame(
  ...
  kbl(table.attr = "data-quarto-disable-processing=true",
      col.names = NULL, align = "l", escape = F,
      caption = paste(IkonPerson_V, "<b>Stævneleder</b><br><i>Ansvarlig på stævnedagen</i>")) %>%
  kable_styling(position = "l", full_width = F, html_font = "verdana") %>%
  column_spec(1, width_min = "5.5em") %>%
  gsub("\\{\\{&lt;|&lt;", "{{<", .) %>% gsub("&gt;}}", ">}}", .)
...
data.frame(
  ...
  kbl(table.attr = "data-quarto-disable-processing=true",
      col.names = NULL, align = "l", escape = F,
      caption = paste(IkonLokation_V, "<b>Lokation</b><br><i>Geografisk placering</i>")) %>%
  kable_styling(position = "l", full_width = F, html_font = "verdana") %>%
  column_spec(1, width_min = "5.5em") %>%
  gsub("\\{\\{&lt;|&lt;", "{{<", .) %>% gsub("&gt;}}", ">}}", .)
```
