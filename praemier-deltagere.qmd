...
DataDeltAndetAkt_T %>%
  ...
  add_header_above(
    ...
    extra_css = "border:hidden;border-bottom:1.5px solid #111111") %>%
  row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
  row_spec(which(grepl("Afbud", DataDeltAndetAkt_T$OrdreStatusSimpel_RD)),
...
```{r}
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpel_RD)) %>% summarise(length(unique(Klub_RD))))+1.5)/3.3
...
"<b>", DataBilletAkt_T$StatDeltKlubAntal_DW, "</b>",
...
DataDeltKlub_T %>%
	...
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltKlub_T$OrdreStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
...
DataDeltBy_T %>%
	...
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltBy_T$OrdreStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
	column_spec(1, width_min = "2em") %>%
	column_spec(1:3, extra_css = "border-top:0.7px solid #999999") %>%
	remove_column(4:5) %>%
...
```{r}
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpel_RD)) %>% summarise(length(unique(DeltAlderKat_RD))))+1.5)/3.3
DataAkt_T %>%
	filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpel_RD)) %>%
...
DataDeltAlderKat_T %>%
	...
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltAlderKat_T$OrdreStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
	column_spec(1, width_min = "2em") %>%
	column_spec(1:3, extra_css = "border-top:0.7px solid #999999") %>%
	remove_column(4:5) %>%
...
```{r}
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpel_RD)) %>% summarise(length(unique(DeltKoen_RD))))+1.5)/3.3
...
DataDeltKoen_T %>%
	...
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltKoen_T$OrdreStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
	column_spec(1, width_min = "2em") %>%
...
```{r}
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpel_RD)) %>% summarise(length(unique(DeltGenKat_DW))))+1.5)/3.3
DataAkt_T %>%
	filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpel_RD)) %>%
...
cat(paste0(
"<i style=font-size:70%>",
"<b>", DataBilletAkt_T$StatDeltGenAntal_DW, "</b>",
...
DataDeltGenTil_T %>%
	...
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltGenTil_T$OrdreStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
	column_spec(1, width_min = "2em") %>%
	column_spec(1:3, extra_css = "border-top:0.7px solid #999999") %>%
	remove_column(4:5) %>%
...
DataDeltOrdreKat_T %>%
	...
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltOrdreKat_T$OrdreStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
	column_spec(1, width_min = "2em") %>%
