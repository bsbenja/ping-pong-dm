...
if(nrow(DataPraemieYngstAeldstAkt_T) == 0) {
  ...
  DataPraemieYngstAeldstAkt_T %>%
    ...
    footnote(ifelse(
      DataBilletAkt_T$DeltAlderForskelAntal_DW == 0, "<i style=font-size:80%>Ingen aldersforskel.</i>",
      paste("<i style=font-size:80%>Aldersforskel", DataBilletAkt_T$DeltAlderForskelAntal_DW, "år.</i>")),
      general_title = "", escape = F) %>%
    remove_column(5) %>%
    gsub("\\{\\{&lt;|&lt;", "{{<", .) %>% gsub("&gt;}}", ">}}", .)
}
