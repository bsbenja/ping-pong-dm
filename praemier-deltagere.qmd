...
if(nrow(DataPraemieYngstAeldstAkt_T) == 0) {
  ...
  DataPraemieYngstAeldstAkt_T %>%
    kbl(col.names = NA, align = "lclc", escape = F,
      caption = paste(
      IkonGave_V, "Gave til yngste- og ældste Ping Pong deltager")) %>%
    kable_styling(position = "l", full_width = F) %>%
    row_spec(0, background = DataEventAarAkt_T$EventAarFarve1_RD, color = "#FFFFFF") %>%
    column_spec(1, bold = T) %>%
    column_spec(1:4, extra_css = "border-top:0.7px solid #999999") %>%
    footnote(paste(
      "Gaverne har til formål at hylde mangfoldigheden.",
      "<br>",
      "Aldersforskel", DataBilletAkt_T$StatAlderForskelAntal_DW, "år."),
      general_title = "", escape = F) %>%
    remove_column(5)
}
```
  ...
  DataDeltForAkt_T %>%
    kbl(col.names = NA, align = "ccl", escape = F,
      caption = paste0(
        ...
        DataEventAarAkt_T$EventAarFristDato_DW_DMAA_DW, ".",
        "<br>",
        "<br>",
...
if(InputInfo1234_V %in% c(1, 2, 4)) {
  ...
  DataDeltPuljerAkt_T %>%
    kbl(col.names = NA, align = "cclr", escape = F,
      caption = paste0(
        IkonPingPong_V, " ",
        DataBilletAkt_T$BilletPuljeDelt_RD, "-mandspuljer til ", DataEventAarAkt_T$EventAar_RD,
        "<br>",
  ...
  row_spec(which(DataDeltPuljerAkt_T$DeltSnakeSeedLagNr_DW == "1"), bold = T,
           extra_css = "border-top:0.7px solid #999999") %>%
  remove_column(5:10)
  }
}
...
if(InputInfo1234_V %in% c(1, 2, 3, 4)) {
  ...
  DataDeltAndetAkt_T %>%
    kbl(col.names = NA, align = "ccl", escape = F,
  		caption = paste0(
        "<p>Andet end Ping Pong + evt. afbud</p>",
...
if(nrow(DataResultSidst_T) == 0) {
  ...
  kbl(col.names = NA, align = "clcl", escape = F,
      caption = paste(
        IkonPokal_V, "DM-resultater sidste <q>Final 4</q>",
        "<br>",
