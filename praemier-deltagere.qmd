...
```{r}
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD)) %>% summarise(length(unique(BilletTilKat_DW))))+1.5)/3.3
DataAkt_T %>%
  filter(!is.na(DeltID_RD)) %>%
  ggplot(mapping = aes(y = fct_rev(BilletTilKat_DW), fill = BilletKat_DW)) +
