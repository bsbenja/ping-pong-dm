...
```{r}
#| output: asis
cat("![](filer/generelt/wcpp-finale-2020.mp4){width=40em}<br><figcaption><i style=font-size:80%>Videoklip af duel fra WCPP-finalen 2020. Kampen endte 15-14 i afgørende sæt til Andrew Baggaley, England mod Alexander Flemming, Tyskland. Hele kampen kan ses på Youtube [<b>HER</b>](https://www.youtube.com/watch?v=q2f9eDg8WHo){target=_blank}.</i></figcaption><p><p>")
```
...
```{r}
#| output: asis
cat("Tidligere kampe ved WCPP kan ses på [Matchroom Live](https://matchroom.live/sports/ping_pong){target=_blank} (gratis bruger kan oprettes) eller YouTube-kanalen [Matchroom Multi Sport](https://www.youtube.com/channel/UCQ8cM7o5Gu9l9G3fDP5Xpsg){target=_blank}.")
```
:::
...
data.frame(
  ...
  row_spec(1, bold = T, background = FarveBG_V, color = "#000000") %>%
  column_spec(3, italic = T, color = FarveFG_V)
