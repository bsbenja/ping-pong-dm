...
#| label: fig-klubber
#| fig-cap: "Klubber."
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>% summarise(length(unique(Klub_ID))))+1.5)/3.3
DataAkt_T %>%
...
#| fig-height: 5
DataAkt_T %>%
  filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD) & !grepl("Ingen klub|Udlandet", Klub_ID)) %>%
...
#| label: fig-koen
#| fig-cap: "Køn."
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>% summarise(length(unique(DeltKoen_ID))))+1.5)/3.3
DataAkt_T %>%
	filter(!is.na(DeltID_RD) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>%
...
#| label: fig-ordre-kat
#| fig-cap: "Ordretype."
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(DeltID_RD)) %>% summarise(length(unique(OrdreKat_ID))))+1.5)/3.3
DataAkt_T %>%
