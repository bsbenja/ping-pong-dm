...
DataResultSidst_T <- tbl3_Result_T %>% filter(
  grepl(InputEventAarAkt_V, EventAarSidst_DW) &
  grepl(BillettypeAkt_V, Billettype_DW) &
  grepl("A-slutspil", DeltSlutspil_ID) &
...
Præmier og deltagere opdateres løbende for `r tbl2_EventAarAkt_T$EventAar_ID` og er sidst opdateret `r format(floor_date(Sys.time(), "30 minutes"), "%d.%m.%Y kl. %H:%M.")`
:::
...
if(InputInfo1234_V %in% c(1, 2, 4)) {
  ...
  DataDeltPuljerAkt_T %>%
    kbl(col.names = NA, align = "cclr", escape = F,
      caption = paste0(
        IkonPingPong_V, " ", DataBilletAkt_T$BilletPuljeDelt_RD, "-mandspuljer til ", tbl2_EventAarAkt_T$EventAar_ID,
        "<br>",
...
#| label: fig-klubber
#| fig-cap: "Klubber."
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>% summarise(length(unique(Klub_ID))))+1.5)/3.3
DataAkt_T %>%
	filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>%
	mutate(Klub_ID = paste0(Klub_ID, "&emsp;", KlubLogo_DW)) %>%
	ggplot(mapping = aes(y = fct_rev(fct_infreq(Klub_ID)), fill = BilletKat_DW)) +
...
#| fig-height: 5
DataAkt_T %>%
  filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD) & !grepl("Ingen klub|Udlandet", Klub_ID)) %>%
	mutate(across("KlubPostnr_RD", \(x) as.integer(x))) %>%
	distinct(Deltager_ID, .keep_all = T) %>%
	mutate(KlubRegion_ID = case_when(
		KlubRegion_ID == "Nordjylland" ~ "nordjylland",
		KlubRegion_ID == "Midtjylland" ~ "midtjylland",
		KlubRegion_ID == "Syddanmark"  ~ "syddanmark",
		KlubRegion_ID == "Sjælland"    ~ "sjaelland",
		KlubRegion_ID == "Hovedstaden" ~ "hovedstaden")) %>%
...
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>% summarise(length(unique(DeltAlderKat_RD))))+1.5)/3.3
DataAkt_T %>%
	filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>%
...
#| label: fig-koen
#| fig-cap: "Køn."
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>% summarise(length(unique(DeltKoen_ID))))+1.5)/3.3
DataAkt_T %>%
	filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>%
	ggplot(mapping = aes(y = fct_rev(fct_infreq(DeltKoen_ID)), fill = BilletKat_DW)) +
	geom_bar(width = 0.9, position = position_stack(reverse = T)) +
	geom_text(aes(label = after_stat(count)), stat = "count", position = position_stack(reverse = T),
						vjust = 0.4, hjust = 1, size = 5, color = "#FFFFFF", fontface = "bold") +
	scale_fill_manual(values = levels(DataAkt_T$BilletKatFarve_DW))
...
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>% summarise(length(unique(DeltGen_DW))))+1.5)/3.3
DataAkt_T %>%
	filter(!is.na(Deltager_ID) & grepl("Tilmeldt", OrdreStatusSimpelKat_RD)) %>%
...
#| label: fig-ordre-kat
#| fig-cap: "Ordretype."
#| fig-height: !expr (as.numeric(DataAkt_T %>% filter(!is.na(Deltager_ID)) %>% summarise(length(unique(OrdreKat_ID))))+1.5)/3.3
DataAkt_T %>%
  filter(!is.na(Deltager_ID)) %>%
  ggplot(mapping = aes(y = fct_rev(OrdreKat_ID), fill = BilletKat_DW)) +
