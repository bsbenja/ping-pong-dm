...
#| output: asis
cat(paste0(
"<i style=font-size:80%>", DataBilletAkt_T$StatDeltRatingKatAntal_DW, "<br>", DataBilletAkt_T$StatDeltRatingAntal_DW, "<br>", DataBilletAkt_T$StatForskudtTilAntal_DW, "<br>", DataBilletAkt_T$StatOekonomiAntal_DW, "</i>", "<br>", DataEventAarAkt_T$EventAarAabningDatoTid_RD))
```
...
```{r}
DataPraemiePenge_T %>%
  filter(EventAar_RD == InputEventAarAkt_V) %>%
...
if(nrow(DataKlubMangel_T) == 0) {
	...
	DataKlubMangel_T %>%
		kbl(table.attr = "data-quarto-disable-processing=true",
		    ...
		    caption = paste0(
					IkonPerson_V, " <b>Manglende klubber</b>",
					"<br>",
		...
		row_spec(which(grepl("Afbud", DataKlubMangel_T$OrdreStatusSimpel_RD)),
...
if(nrow(DataDeltMangel_T) == 0) {
  ...
  DataDeltMangel_T %>%
	kbl(table.attr = "data-quarto-disable-processing=true",
	    col.names = NA, align = "l", escape = F,
			caption = paste0(
				IkonPerson_V, " <b>Manglende deltagere</b>",
				"<br>",
				"<i style=font-size:80%>Sorteret efter klubantal og år</i>")) %>%
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltMangel_T$OrdreStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
	column_spec(1, width_min = "2em") %>%
