...
DataDeltMangel_T <- tbl1_Ordre_T %>%
	anti_join(y = DataAkt_T, by = "Delt_ID") %>%
	group_by(EventAar_RD, Delt_ID) %>%
	slice(which.max(OrdreDatoTid_RD)) %>%
	group_by(EventAar_RD) %>%
	add_count(Klub_ID) %>%
	arrange(
...
[**Sisu/MBK**]{.underline}:\*\*\*\* ✅ Puljespil: 6-mandspuljer på maksimalt 18 borde. 2 puljer spilles på 3 borde. 30 kampe spilles på 3 borde = 10 kampe pr. bord (4 timer). Efterfølgende tages bordene gradvist ned.\
