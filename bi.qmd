...
Plot.plot({
  ...
  marks: [
    ...
    Plot.groupY({x: "count"},
    {y: "BilletTilKat_DW", fill: "BilletKat_DW"}
    )
    )
  ]
})
...
if(nrow(DataKlubMangel_T) == 0) {
	...
	DataKlubMangel_T %>%
		...
		row_spec(which(grepl("Afbud", DataKlubMangel_T$BilletStatusSimpel_RD)),
...
DataDeltMangel_T <- Data_T %>%
	...
	group_by(BilletStatusSimpel_RD) %>%
	mutate(Nr. = row_number()) %>%
	select(
		Nr.,
		" "     = KlubLogo_DW,
...
if(nrow(DataDeltMangel_T) == 0) {
  ...
  DataDeltMangel_T %>%
	...
	kable_classic(position = "l", full_width = F, html_font = "verdana") %>%
	row_spec(0, background = unique(DataEventAarAkt_T$EventAarFarve1_RD), color = "#FFFFFF") %>%
	row_spec(which(grepl("Afbud", DataDeltMangel_T$BilletStatusSimpel_RD)),
					 strikeout = T, color = unique(DataEventAarAkt_T$EventAarFarve1_RD)) %>%
	column_spec(1, width_min = "2em") %>%
