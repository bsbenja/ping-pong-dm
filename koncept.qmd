...
DataEventAarSidst_T <- tbl2_EventAar_T %>% filter(grepl(InputEventAarAkt_V, EventAarSidst_DW))
DataResultSandpapirAabenSingle_T <- tbl3_Result_T %>% filter(
  (grepl("A-slutspil", DeltSlutspil_ID) &
  grepl("1", DeltPlacNr_RD) &
  grepl("Sandpapir - Åben - Single", Billettype_DW)) |
...
::: {#fig-cover}
![](filer/medie/billede/wcpp-finale-2019.png){width="40em"}
...
`r tbl2_EventAarAkt_T$InfoTipRegler_DW`
:::
...
`r tbl2_EventAarAkt_T$InfoTipVM_DW`
:::
...
![](filer/medie/billede/bat-sandpapir-sanwei.png){width="15em"}
...
![](filer/medie/billede/bat-sandpapir-carlton.png){width="15em"}
...
![](filer/medie/billede/bat-sandpapir-dunlop.png){width="15em"}
...
```{r}
#| label: tbl-dm-statistik-sandpapir-aaben-single
...
if(nrow(DataResultSandpapirAabenSingle_T) == 0) {
  kbl3_dm_resultater <- data.frame() %>% kbl()
} else {
DataResultSandpapirAabenSingle_T %>%
  kbl(col.names = NA, align = "clcl", escape = F,
      caption = paste(IkonPokal_V, "DM-vindere Sandpapir Åben Single")) %>%
  kable_styling(position = "l", full_width = F, htmltable_class = "table-condensed") %>%
  row_spec(
    which(grepl("Aflyst", DataResultSandpapirAabenSingle_T$OrdreStatusSimpelKat_RD)),
    strikeout = T, italic = T, color = TabelFarveFG_V) %>%
  remove_column(c(2, 5:10))
}
```
